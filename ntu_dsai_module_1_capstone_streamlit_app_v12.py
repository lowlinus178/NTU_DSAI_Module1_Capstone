# ntu_dsai_module_1_capstone_streamlit_app_v12.py
...
def load_and_prepare_data():
    try:
        url = "https://onedrive.live.com/download?cid=6b8eb3e2d12f8aa7&resid=6B8EB3E2D12F8AA7!s7ce91eeed0e3458cbea4623f9d39d9ea"
        df = pd.read_excel(url)
    except Exception as e:
        st.warning("Could not fetch from OneDrive, using sample CSV instead.")
        df = pd.read_csv("SGJobData_cleaned_processed.csv")
...
fig_ts = go.Figure()
fig_ts.add_scatter(x=ts['month_year'], y=ts['job_postings'], name="Job Postings", line=dict(color='#58a6ff', width=3))
fig_ts.add_scatter(x=ts['month_year'], y=ts['applicants'], name="Applicants", yaxis="y2", line=dict(color='#ff7b72'))
fig_ts.add_scatter(x=ts['month_year'], y=ts['vacancies'], name="Vacancies", yaxis="y2", line=dict(color='#3fb950'))
...
fig_comp.add_trace(go.Bar(x=b10_cats, y=global_avg_stats.loc[b10_cats], marker_color='#ff7b72'), row=1, col=1)
fig_comp.add_trace(go.Bar(x=t10_cats, y=global_avg_stats.loc[t10_cats], marker_color='#3fb950'), row=1, col=2)
...
fig_ratio.add_trace(go.Bar(x=b10_cats, y=global_ratio_stats.loc[b10_cats], marker_color='#58a6ff'), row=1, col=1)
fig_ratio.add_trace(go.Bar(x=t10_cats, y=global_ratio_stats.loc[t10_cats], marker_color='#58a6ff'), row=1, col=2)
